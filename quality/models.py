...
class MaterialTestResult(AbstractEntity):
    ...
    judged_upper_limit0 = models.DecimalField(help_text='判定基准上限', decimal_places=3, max_digits=8, default=0)
    judged_lower_limit0 = models.DecimalField(help_text='判定基准下限', decimal_places=3, max_digits=8, default=0)
...
class MaterialDealResult(AbstractEntity):
    ...
    test_result = models.CharField(max_length=64, help_text="综合检测结果，一等品/三等品/PASS/试验")
    ...
    desc = models.CharField(max_length=64, help_text='备注', blank=True, null=True)
...
class UnqualifiedDealOrder(AbstractEntity):
    ...
    t_deal_date = models.DateTimeField(help_text='技术日期', blank=True, null=True)
    c_deal_user = models.CharField(max_length=64, help_text='检查部门处理人', blank=True, null=True)
    c_deal_date = models.DateTimeField(help_text='检查日期', blank=True, null=True)
