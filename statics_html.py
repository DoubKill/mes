...
if __name__ == '__main__':
    ...
    # pie = Pie()
    equip_list = list(Equip.objects.filter().values_list("equip_no", flat=True))
    equip_list.sort()
    # 统计各个机台各个部位的故障比例
    ...
    for temp in temp_set:
        equip_part_dict[temp.get("equip_part__equip__equip_no")][0].append(temp.get("equip_part__name"))
        equip_part_dict[temp.get("equip_part__equip__equip_no")][1].append(temp.get("counts"))
    count = len(equip_list)
    pie = Pie(init_opts=opts.InitOpts(width='1250px', height='800px', page_title="设备运行统计"))
    x = 125
    y = 150
    for k, v in equip_part_dict.items():
        if v[0] and v[1]:
            data = [list(z) for z in zip(v[0], v[1])]
            pie.add(series_name=k, data_pair=data, center=[x, y], radius="100", label_opts=opts.LabelOpts())
            x += 250
            if x > 1200:
                x = 125
                y += 250
    ...
    pie.set_global_opts(title_opts=opts.TitleOpts(title="机台维修时间占比", subtitle='单位：%'),
                         legend_opts=opts.LegendOpts())
