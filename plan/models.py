...
class SchedulingParamsSetting(models.Model):
    ...
    scheduling_adaptable_trains = models.IntegerField(help_text='现场可修改计划数量（车）')
    min_stock_trains = models.FloatField(help_text='确保最低库存量（天）')
    scheduling_auto_time = models.CharField(max_length=32, help_text='排程参数自动统计时间')
    standing_time = models.IntegerField(help_text='无S打加S放置期时间（小时）')
    pkg_count = models.IntegerField(help_text='小料包一车包数')
    validity = models.IntegerField(help_text='小料包有效期（小时）')
    mixing_summary_st_time = models.IntegerField(help_text='开始密炼时间统计范围（秒）')
    mixing_summary_et_time = models.IntegerField(help_text='结束密炼时间统计范围（秒）')
    mixing_interval_st_time = models.IntegerField(help_text='开始密炼间隔时间统计范围（秒）')
    mixing_interval_et_time = models.IntegerField(help_text='结束密炼间隔时间统计范围（秒）')
