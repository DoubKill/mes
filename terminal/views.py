...
class XlRecipeNoticeView(APIView):
    ...
    def post(self, request):
        ...
        if '成功' in detail_msg:
            ...
            record = RecipeChangeDetail.objects.filter(change_history__recipe_no=product_no.split('_NEW')[0], change_history__dev_type=product_batching.dev_type.category_name,).last()
            if record:
