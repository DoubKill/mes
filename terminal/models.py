...
class WeightBatchingLog(AbstractEntity):
    ...
    material_name = models.CharField(max_length=64, help_text='原材料名称', null=True, blank=True)
    material_no = models.CharField(max_length=64, help_text='原材料编码', null=True, blank=True)
    bra_code = models.CharField(max_length=64, help_text='条形码', null=True, blank=True)
    status = models.PositiveIntegerField(help_text='状态', choices=STATUS_CHOICE, default=1)
    batch_time = models.DateTimeField(max_length=64, help_text='投入时间', null=True, blank=True)
    batch_classes = models.CharField(max_length=64, help_text='投入班次', null=True, blank=True)
    batch_group = models.CharField(max_length=64, help_text='投入班组', null=True, blank=True)
    tank_no = models.CharField(max_length=64, help_text='投入罐号', null=True, blank=True)
    location_no = models.CharField(max_length=64, help_text='产线', null=True, blank=True)
    dev_type = models.CharField(max_length=64, help_text='机型名称', null=True, blank=True)
    failed_reason = models.CharField(max_length=64, help_text='投料失败原因', null=True, blank=True)
    open_time = models.CharField(max_length=19, help_text='开门时间', default='')
