        ...
        if my_order == '1':
            if not status:
                ...
                query_set = self.queryset.filter(Q(assign_to_user__icontains=user_name) | Q(receiving_user=user_name) |
                                                 Q(repair_user__icontains=user_name) | Q(status=status))
        elif my_order == '2':
            if not status:
                if not searched:
                    query_set = self.queryset.filter(Q(Q(status='已接单') |
...
class EquipOrderEntrustView(APIView):
    ...
    def get(self, request):
        ...
        user_name = self.request.user.username
        if oper_type == '维修':
            apply_order = EquipApplyOrder.objects.filter(Q(status='已开始') | Q(status='已接单'), repair_user__icontains=user_name).order_by('entrust_to_user', '-last_updated_date')
