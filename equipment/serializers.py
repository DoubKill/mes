...
class EquipMaintenanceAreaSettingSerializer(BaseModelSerializer):
    ...
    def validate(self, attrs):
        i = EquipMaintenanceAreaSetting.objects.filter(equip=attrs['equip'], maintenance_user=attrs['maintenance_user'], equip_part=attrs['equip_part'])
        if i:
            raise serializers.ValidationError('请勿重读添加')
        return attrs
...
class CheckTemperatureTableSerializer(BaseModelSerializer):
    ...
    class Meta:
        ...
        validators = [UniqueTogetherValidator(queryset=CheckTemperatureTable.objects.filter(delete_flag=False),
                                              fields=('select_date', 'classes', ''), message='已经存在当前日期温度检查表')]
