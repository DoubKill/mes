...
from production.filters import TrainsFeedbacksFilter, PalletFeedbacksFilter, QualityControlFilter, EquipStatusFilter, \
    ...
    ProductInfoDingJiFilter, SubsidyInfoFilter, PerformanceJobLadderFilter, AttendanceGroupSetupFilter
from production.models import TrainsFeedbacks, PalletFeedbacks, EquipStatus, PlanStatus, ExpendMaterial, OperationLog, \
    ...
    FillCardApply, ApplyForExtraWork
from production.serializers import QualityControlSerializer, OperationLogSerializer, ExpendMaterialSerializer, \
    ...
    FillCardApplySerializer, ApplyForExtraWorkSerializer
...
class EmployeeAttendanceRecordsView(APIView):
    ...
    def post(self, request):
            ...
            for names in data:
                ...
                for i, name in enumerate(names):
                    if name:
                        ...
                        dic = {'name': name, 'section': section, 'date': date_, 'group': group, 'equip': equip}
                        dic2 = dic.copy()
                        dic2.pop('name')
                        if dic in records:
                            continue
        ...
        except: raise ValidationError('导入文件格式有误')
...
class EmployeeAttendanceRecordsExport(ViewSet):
    ...
    def export(self, request):
        ...
        # 获取设备对应的机型
        equip_list = Equip.objects.filter(category__equip_type__global_name__in=['密炼设备', '称量设备']).order_by('equip_no').values('equip_no', 'category__category_name')
        # dic = {item['equip_no']: item['category__category_name'] for item in equip_list}
        # 获取设备对应的岗位
        section_list = PerformanceJobLadder.objects.filter(delete_flag=False).values('type', 'name')
...
class PerformanceSummaryView(APIView):
    ...
    def get(self, request):
        ...
        # user_dic  {1_a班_投料_Z02_早班：{'name: '张三', 'section': '挤出', 'equip_no': Z01, 'classes': '早班'}}
        ...
        for item in equip_max_value:
            if not max_value.get(f"{item['equip_no']}"):
                max_value[f"{item['equip_no']}"] = item['qty']
        # 取每个机台设定的目标值
        ...
        for k, v in results1.items():
            section, name, day, group = k.split('_')[-1], k.split('_')[0], k.split('_')[1], k.split('_')[2]
                ...
                for item in v[1:]:
                    if post_standard == 1:  # 最大值
                        for key in item.keys():
                            if key.split('_')[0] in state_list:
                                if dic.get(key) and dic.get(key) < item[key]:
                                    dic[key] = item[key]
                    else:  # 平均值
                        for key in item.keys():
                            if dic.get(key) and key.split('_')[0] in state_list:
                                dic[key] += item[key]
                            else:
                                dic[key] = item[key]
                if post_standard == 2:
        ...
        if day_d and group_d:
                ...
                for k, v in dic.items():
                    if equip in equip_list:  # 称量
                        ...
                        if k.split('_')[-1] == 'qty':
                            ...
                            key_unit = '_'.join(k.split('_')[0:-1]) + '_unit'  # dic[key_unit]
                            if detail.get(key):
                                detail[f"{equip}-车数"][state] = v
                                detail[f"{equip}-单价"][state] = dic[key_unit]
                                detail[f"{equip}-岗位"][state] = section
                            else:
                                detail.update({
                                    f"{equip}-车数": {'name': f"{equip}-车数", state: v},
                                    f"{equip}-单价": {'name': f"{equip}-单价", state: dic[key_unit]},
                        ...
                        if k.split('_')[-1] == 'qty':
                            ...
                            key = f"{equip}{type}-车数"
                            key_unit = '_'.join(k.split('_')[0:-1]) + '_unit'
                            if detail.get(key):
                                detail[f"{equip}{type}-车数"][state] = v
                                detail[f"{equip}{type}-单价"][state] = dic[key_unit]
                                detail[f"{equip}{type}-岗位"][state] = section
                            else:
                                detail.update({
                                    f"{equip}{type}-车数": {'name': f"{equip}{type}-车数", state: v},
                                    f"{equip}{type}-单价": {'name': f"{equip}{type}-单价", state: dic[key_unit]},
            ...
            return Response({'results': detail.values(), 'hj': hj, 'all_price': round(all_price['price'] * a, 2), '超产奖励': res['超产奖励']})
...
class IndependentPostTemplateView(APIView):
    ...
    def get(self, request):
        ...
        if export:
            names = EmployeeAttendanceRecords.objects.filter(date__year=year, date__month=month).values_list('name', flat=True).distinct()
            if names:
...
class AttendanceClockViewSet(ModelViewSet):
    # AttendanceGroupSetup 考勤组管理
    # EmployeeAttendanceRecords  考勤记录
    queryset = EmployeeAttendanceRecords.objects.all()
    serializer_class = EmployeeAttendanceRecordsSerializer
    # filter_backends = (DjangoFilterBackend,)
    # filter_class = EmployeeAttendanceRecordsFilter
    ...
    def get_user_group(self, user_obj):
        ...
        # 获取登陆用户所在考勤组
        attendance_group_obj = AttendanceGroupSetup.objects.filter(attendance_users__icontains=username).first()
    ...
    def list(self, request, *args, **kwargs):
        ...
        results = {
            ...
            # 'attendance_st': attendance_group_obj.attendance_st,
            # 'attendance_et': attendance_group_obj.attendance_et,
            # 'range_time': attendance_group_obj.range_time,
            # 'lead_time': attendance_group_obj.lead_time
        }
        if apply:  # 补卡/加班
            return Response({'results': results})
    ...
    def create(self, request, *args, **kwargs):
        ...
        if status == '上岗':
            ...
            EmployeeAttendanceRecords.objects.filter(id__in=ids).update(
                end_date=end_date, work_time=work_time
            )
            results['ids'] = ids
        elif status == '换岗':
            # begin_date = EmployeeAttendanceRecords.objects.filter(id__in=ids).first().begin_date
            end_date = begin_date = time_now
            work_time = round((end_date - begin_date).seconds / 3600, 2)
            EmployeeAttendanceRecords.objects.filter(id__in=ids).update(
                end_date=end_date, work_time=work_time
            )
        ...
        # status  上岗/换岗/
        # 1.上岗 begin_date
        # 2.换岗 默认当前时间就是下岗时间(end_time)，计算work_time，然后新建一条
        # 3.下岗  默认当前时间就是下岗时间(end_time)，计算work_time
        # 4. 补卡/换岗/加班记录履历
        return Response({'results': results})
    ...
    def reissue_card(self, request):
        ...
        content = {
            ...
            "form": [{"key": "姓名:", "value": username},
                     {"key": "身份证号:", "value": user.id_card_num},
                     ...
                     {"key": "处理意见:", "value": apply.handling_suggestion},
                     {"key": "处理结果:", "value": apply.handling_result}]},
    ...
    def overtime(self, request):
        ...
        content = {
            ...
            "form": [{"key": "姓名:", "value": user.username},
                     {"key": "身份证号:", "value": user.id_card_num},
                     {"key": "班组:", "value": apply.group},
                     {"key": "班次:", "value": apply.classes},
                     {"key": "岗位:", "value": apply.section},
...
class AttendanceRecordSearch(APIView):
    ...
    def get(self, request):
        ...
        for item in queryset.filter(factory_date__year=year, factory_date__month=month):
            if item.factory_date__day not in results.get('days'):
                attendance_group_obj = AttendanceGroupSetup.objects.filter(attendance_users__icontains=username).first()
                begin_date = min(queryset.filter(factory_date=item.factory_date).values_list('begin_date', flat=True))
                end_date = max(queryset.filter(factory_date=item.factory_date).values_list('end_date', flat=True))
                work_time = round((end_date - begin_date).seconds / 3600, 2)
                results['work_times'] += work_time
                if begin_date > attendance_group_obj.attendance_st:
                    results['cd'] += 1
                if end_date < attendance_group_obj.attendance_et:
                    results['zt'] += 1
        if detail:
            equip_list = queryset.values('equip', 'section').annotate(work_times=Sum('work_time')).values(
                'equip', 'section', 'work_times')
            results['equip_list'] = equip_list
            work_detail = queryset.values('factory_date', 'classes', 'group', 'equip', 'section', 'work_time')
